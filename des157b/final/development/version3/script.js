// JS here
const TIME_BETWEEN_SAVES = 10;
let category = "";
let curInput;

function loadLocalStorage(category, input) {
  let localData = JSON.parse(localStorage.getItem(`For Us - ${category}`));

  if (localData !== null) {
    for (let i = 0; i < input.length; i++) {
      let key = input[i].getAttribute("name");
      if (localData[key] !== undefined) {
        input[i].value = localData[key];
      }
    }
  }
}

function saveLocalStorage(category, input) {
  let localData = JSON.parse(localStorage.getItem(`For Us - ${category}`));
  let categoryData = localData == null ? {} : localData;
  ...
  localStorage.setItem(`For Us - ${category}`, JSON.stringify(categoryData));
}

setInterval(() => {
  if (category != "") {
    saveLocalStorage(category, curInput);
  }
}, TIME_BETWEEN_SAVES * 1000);
...
(function () {
  ...
  newButton4.addEventListener("click", function (event) {
    ...
    // LocalStorage
    loadLocalStorage("ghosts", inputs4);
    category = "ghosts";
    curInput = inputs4;
  });
  ...
  async function addFriend() {
    ...
    if (
      ...
      try {
        ...
        resetFormFields();
        localStorage.setItem(`For Us - food`, null);
        addFriendForm.className = "add-friend-offscreen";
  ...
  async function addFriend2() {
    ...
    if (
      ...
      try {
        ...
        resetFormFields();
        localStorage.setItem(`For Us - art`, null);
        addFriendForm2.className = "add-friend-offscreen";
        friendList.innerHTML = "";
        friendList2.innerHTML = "";
        friendList3.innerHTML = "";
  ...
  async function addFriend3() {
    ...
    if (
      ...
      try {
        ...
        resetFormFields();
        localStorage.setItem(`For Us - war`, null);
        addFriendForm3.className = "add-friend-offscreen";
        friendList.innerHTML = "";
        friendList2.innerHTML = "";
        friendList3.innerHTML = "";
  ...
  async function addFriend4() {
    ...
    if (
      ...
      try {
        ...
        resetFormFields();
        localStorage.setItem(`For Us - ghosts`, null);
        addFriendForm4.className = "add-friend-offscreen";
        friendList.innerHTML = "";
        friendList2.innerHTML = "";
        friendList3.innerHTML = "";
